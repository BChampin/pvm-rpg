    ...
    const mapsPromises = alienJson.table.rows[2].c.map(async (cell, cellIndex) => {
      ...
      return {
        ...
        times: {
          ...
          intermediate: timeStrToNumber(noviceJson.table.rows[4].c[cellIndex]?.v),
...
async function generatePlayers() {
  try {
      ...
      for (const [rowIndex, row] of sheetJson.table.rows.entries()) {
        ...
        for (const [cellColIndex, cell] of row.c.entries()) {
          if (cell && cell.v && cellColIndex > 1) {
            ...
            const mapCategory = ['Alien', 'Player', 'Intermediate', 'Noob', 'Not defined'][
              [currentMap.times.alien, currentMap.times.player, currentMap.times.intermediate, currentMap.times.noob]
    ...
    for (const [rowIndex, row] of fameJson.table.rows.entries()) {
      ...
      if (playerRight !== '/') {
        const findPlayerIndex = players.findIndex(p => p.name === playerRight)
        if (findPlayerIndex) {
          players[findPlayerIndex].rawFame = {
